...
|Event||
|--|--|
|MGOS_EV_BTHING_SHADOW_CHANGED|Triggered when the shadow state is changed. The event-data passed to the handler is a `struct mgos_bthing_shadow_state*`.<br><br>Allowed `state_flags` are: `MGOS_BTHING_STATE_FLAG_CHANGED` and `MGOS_BTHING_STATE_FLAG_FORCED_PUB`(optional).|
|MGOS_EV_BTHING_SHADOW_UPDATED|Triggered when the shadow state has been updated. It is triggered also if the shadow state is not changed. The event-data passed to the handler is a `struct mgos_bthing_shadow_state*`.<br><br>Allowed `state_flags` are: `MGOS_BTHING_STATE_FLAG_UPDATED`, `MGOS_BTHING_STATE_FLAG_UNCHANGED`(optional), `MGOS_BTHING_STATE_FLAG_CHANGED`(optional) and `MGOS_BTHING_STATE_FLAG_FORCED_PUB`(optional).|
